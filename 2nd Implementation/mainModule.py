...
import SafeMode as sm
import RandomSorter as rs
import search as s
...
def start():
    ...
    while True:
        isValidFN = False
        fileName_list = os.listdir(path_dir)
        fileName_list_lower = [x.lower() for x in fileName_list]
        while not safemode.isApprove() or not isValidFN:
            ...
            if Space_check.search(fileName):
                ...
                if safemode.isEnable():
                    safemode.checkFolderName(fileName)
                else:
                    safemode.setApprove(True)
                isValidFN = True
        safemode.initApprove()
    ##########################

        if fileName not in fileName_list_lower and len(fileName) <= 50 \
        ...
        #파일명 중복의 경우
        elif fileName in fileName_list_lower:
            print("파일명이 중복되었습니다. 다시 지정해주세요.\n")

        #파일명이 0이거나 50이상인 경우
        #elif len(fileName) > 50 or len(fileName) == 0:vvyvy
        #공백에 대한 예외처리 필요...미해결
...
def photochecker(fileName):
    ...
    file_list_photo = [file for file in file_list if file.endswith(".bmp") or
                       file.endswith(".jpeg") or file.endswith(".jpg") or file.endswith(".png") or file.endswith(
        ".gif")]

    oklist = ['OK', 'ok', 'Ok', 'oK']
    while not safemode.isApprove() or not file_list_photo:
        if not file_list_photo:
        ...
        if safemode.isEnable():
            safemode.checkPhotoFile()
            if safemode.isApprove():
                file_list_photo = [file for file in file_list if file.endswith(".bmp") or
                                   file.endswith(".jpeg") or file.endswith(".jpg") or
                                   file.endswith(".png") or file.endswith(".gif")]
        else:
    ...
    safemode.initApprove()
        ...
        # if safemode.isEnable() and file_list_photo:
        #     safemode.checkPhotoFile()
        # else:
        #     print("사진 파일이 확인되었습니다.\n")
        #     safemode.setApprove(True)
        ...
        if check1.match(num) or check2.match(num):
            ...
            if check_duplicate(number_array) is True:
                if safemode.isEnable():
                    safemode.checkMode()
                    if safemode.isApprove():
                        randomSorter.askEnable()
                        if randomSorter.isEnable():
                            while not randomSorter.isApprove():
                                randomSorter.shufflelist(number_array)
                                randomSorter.checkSort(number_array)

                else:
                    print("분류 기준을 설정하였습니다.\n")
                    randomSorter.askEnable()
                    if randomSorter.isEnable():
                        while not randomSorter.isApprove():
                            randomSorter.shufflelist(number_array)
